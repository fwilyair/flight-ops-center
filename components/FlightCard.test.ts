...
test('separates each flight card from the transparent timeline row', async () => {
    ...
    assert.match(rowSource, /className="flight-row group relative flex"/);
    assert.match(indexSource, /\.flight-row \{[\s\S]*?transition-property: height, margin-bottom, opacity;/);
